...
### Fixed
- **A Safari profile whose stored identity carried its window title is no longer rejected forever ([#109](https://github.com/achiya-automation/safari-mcp/issues/109)).** Twice in three days — 2026-09-18 and 2026-09-20 — one profile answered every tool call with `Safari profile extension unavailable` for the better part of an hour while the other three profiles worked and the failing worker was demonstrably alive (its 20-second heartbeat never stopped). Read from inside the running worker: Safari does not load `extension/background.js` as UTF-8. It decodes the file in the system's legacy text encoding — Mac Hebrew on the Mac where this was found, Mac Roman on an English one — so the literal `" — "` in `_canonicalProfileName` arrived as the char codes `20 5d2 20ac 201d 20` and never matched the real em dash in a window title. An identity persisted as `"אוטומציות — עמוד הפתיחה"` therefore never reduced to `"אוטומציות"`, `_verifyProfileMatch` compared the two and refused its own host, and every reconnect refused again. The 2026-09-18 recovery (a sqlite edit that removed the suffix by hand, then a Safari relaunch) worked for exactly that reason and was wrong about why. Every non-ASCII character in the extension's code is now written as an escape (`" \u2014 "`, `\u26a0\ufe0f`), and `test/extension-source-encoding.test.mjs` fails on any raw non-ASCII character outside a comment in `background.js`, `content.js` or `command-content.js` — and proves the defect by running the canonicalizer's own source through a legacy decoder. Found and verified in the running worker by the parallel GBP session on 2026-09-20. Extension 2.10.24.
- **The host names the workers it counts.** `⚠️ 3 extension workers connected but none proved profile` said nothing about which workers those were, and the advice beneath it prescribed the sqlite write above. The host now keeps a ledger of every worker that reaches `/connect`: how often, whether it was rejected (`426 upgrade_required` and `401` used to be silent — a worker Safari kept from an older build, or one whose bundled token no longer matches, retried forever without a trace), what the profile probe answered it (`match`, `wrong:<window>`, `notfound`, `error:<message>`), and when it verified. The warning prints that ledger, each distinct probe verdict is logged once (`Profile probe from worker 99ad2b3f for "אוטומציות": wrong:עבודה — mcp-profile-check-…`), and the verified line carries the worker id. On 2026-09-20 this is what showed the missing profile's worker among 45 "connected, never asked for a probe" entries — a worker that connects and silently rejects, which is exactly what the encoding defect produces.
- The bridge-token resource fetch — the one await before `/connect` with no timeout — is bounded to 5 seconds.

### Added
- **`safari_doctor` reports the extension bridge.** After the permission checklist: whether this process is the extension host, which worker holds the profile lease and when it last polled, the worker ledger above, and what each Safari profile's extension storage holds — stored identity (with its canonical form when a title suffix is present), heartbeat age and badge state — read through `sqlite3 -readonly`, so an outage like #109 is diagnosable from the tool instead of from sqlite by hand. A process that macOS keeps out of Safari's container says so and names the Full Disk Access grant that would open it.

## [2.21.11] - 2026-09-18

### Fixed
- **One slow page no longer freezes every other check in the session — or blocks a second session driving the same Safari.** Three layers each enforced strict one-at-a-time execution. The Swift daemon read one line from stdin, then waited on that command's semaphore for up to 30 seconds before reading the next, so a single `do JavaScript` against a heavy or still-loading page stalled everything behind it, including native clicks and keystrokes that never touch Apple Events. The Node side had to serialize as well, because responses were matched to callbacks by FIFO queue position — correct only with one request in flight. Measured on a busy profile, a trivial `count of windows` returned `AppleScript execution timed out (30s)` for minutes on end while `safari_list_tabs` kept answering, because the extension path was the only one not stuck behind Apple Events. Every request now carries an id the daemon echoes back, replies are matched by identity and may overtake each other, and the mutex lifts as soon as the running daemon proves it speaks the protocol (an older binary keeps the FIFO fallback). A native command now answers in ~0.5s alongside a 3-second script instead of queueing behind it.
- **AppleScript execution stays strictly serial, deliberately.** An intermediate version of the fix ran scripts on a concurrent queue and `NSAppleScript` — which is not thread-safe — handed a 3-second script the *next* script's result under its own request id. Silent cross-talk, the worst possible failure for anything that measures. Execution is pinned to one serial queue; Safari processes Apple Events one at a time regardless, so this costs no real throughput. `test/helper-concurrency.test.mjs` fails on either regression.
- **A busy profile no longer strands the tab `safari_new_tab` just created.** The extension gave Safari 2 seconds to expose a new tab's navigated URL before minting its receipt, and threw `Safari did not expose the new tab navigation before receipt issuance` otherwise — discarding a tab that already existed, was already owned, and whose destination Safari had already accepted. On a loaded profile that left a trail of orphan blank tabs and told the caller creation had failed, which locked a whole session out of opening tabs. Observed in the daemon log afterwards: real `new_tab` calls taking 10.5s, 12.2s and 14.8s, all of which the old budget would have failed. The wait is now 12 seconds (the caller's own budget is 30s, or 75s on a profile), and when even that is exceeded the receipt binds to the requested URL instead of throwing — same-origin navigations match it, and anything landing cross-origin re-mints through `get_tab_receipt` on the still-owned tab.
- **The daemon no longer exits before answering a one-shot invocation.** With replies emitted asynchronously, `echo '{"script":"…"}' | safari-helper` — the shape used by the AppleScript fallback and by ad-hoc probes — closed stdin immediately and the process exited through the read loop's `exit(0)` before the queued script ever responded, so the caller read nothing at all. Shutdown now drains work already in flight, bounded by one script timeout, and returns immediately when nothing is running.
...
### Fixed
- **`safari_reload_extension` no longer times out when the extension worker has gone idle.** A reload prepares a one-shot handoff that pins the active worker's lease, so no other worker can slip in while that worker gets ready to reload. But the pin held even when the active worker had already parked and could never receive the command: the reload itself woke Safari's fresh worker, which was refused every 3s until the reload timed out at 30s. Diagnostic logging showed it live — `verify refused for worker f99a13: lease held by 8a80b9` seven times, then `reload_extension timed out: never delivered; … last poll 56s ago`. A reload that nobody has received no longer pins the lease; the ordinary rule applies (a worker silent for 20s may be replaced), and the undelivered reload moves to the successor, which runs it. A reload already delivered to a worker still fences the lease exactly as before. Covered in `test/profile-extension-routing.test.mjs`.
- **A command sent right after the extension wakes is no longer lost.** Whenever Safari woke the extension worker, a second `connect()` followed a few seconds after the first had already succeeded: the minute alarm scheduled a retry while the startup connect was still in flight, and a successful connect never cancelled that retry. The duplicate invalidated the healthy poll loop, which then returned without running the command it had just been handed, and the host waited out 30s for an answer that could never come. The host's own keepalive check is sent 3s after each verification, so it landed in that window almost every time — the `keepalive tab check skipped: Extension timeout` line that filled the daemon log every minute — and so could the first command after an idle wake. Diagnostic logging caught it exactly: `list_tabs delivered 30s ago to worker a65e8e … last heartbeat never`, between two verifications of that worker four seconds apart. A successful connect now cancels any pending retry, the alarm does not schedule one while a connect is in flight, and a loop that a reconnect invalidates still runs a command it already received. `test/reconnect-drops-command.test.mjs` covers all three.
...
- **The extension no longer answers every command a second time, five and a half minutes later.** Each command races a 330s wedge guard so a command that never settles cannot park the poll loop, but the guard's timer was never cancelled when the command finished normally. 330s after every command the worker posted "command did not settle" for an id the host had already answered, and the host logged a result for an unknown request each time. The poll loop now cancels the guard however the command ends; `test/long-command-heartbeat.test.mjs` runs the guard with fake timers and checks that a settled command gets no second answer while a wedged one gets exactly one. Manifest 2.10.16.
- **A refused tab receipt now says which check refused it, and only an origin change is told to rotate with `getReceipt`.** `_resolveReceiptTab` refuses for five different reasons — the extension holds no record of the token, the record is from an earlier browser session, its tab is closed, the tab has moved to a different page that no session owns, or the tab has left the receipt's origin — and all five came back as one sentence, "receipt is forged, stale, ambiguous, or not valid for this origin". The server matches "not valid for this origin" and appended "Likely cause: the tab moved to another origin … Rotate it with getReceipt" to every one of them. On 15.9.26 a receipt minted at 09:34:38 was refused after a daemon restart at 09:35:27 while its tab sat unchanged on the same page: the advice pointed at an origin change that never happened, `getReceipt` was refused with the same sentence, and "ambiguous" pointed at a second tab open at the same URL — which cannot make a receipt ambiguous: since 62644b9 a receipt resolves by its tab id and no URL is compared across tabs. Reproducing it showed the restart is not the cause: with one extension worker, receipts for a single tab and for two identical-URL tabs all resolved to their own tab after clean daemon restarts, and kept resolving across six worker restarts. The one condition the reproduction lacked is that at 09:34 Xcode's DerivedData build product was registered as a second copy of the extension; a copy that did not issue a receipt cannot resolve it, and the refusal now says that ("never issued by this extension instance") instead of blaming the origin. Each refusal names its check, only the origin refusal still carries the phrase the server decorates, and `test/receipt-refusal-reason.test.mjs` runs the real resolver over every refusal path, checks that the server's advice matches the origin case alone, and resolves two same-URL tabs to their own receipts. Manifest 2.10.17 — rebuild the extension app to pick it up.

## [2.21.7] - 2026-09-15

### Fixed
- **`safari_emulate` emulates the devices in people's hands on iOS 27, and the emulation now survives its own tool call.** The presets were iPhone 14 / iOS 17 strings, but since iOS 26 Safari freezes the OS token of its user agent (`iPhone OS 18_7`) and only `Version/` moves, and iPadOS requests desktop sites, so an iPad sends the Mac user agent and is told apart only by `navigator.maxTouchPoints`. New presets — iPhone 18 Pro, 18 Pro Max, Air, 17, 17e, 16; iPad (A16), iPad Air 11/13, iPad Pro 11/13 — carry the viewport, user agent, `platform` and touch points read from Mobile Safari on the iOS/iPadOS 27.0 simulator; the older names still resolve. The override itself never stuck: the tool set it and then reloaded the page, which hands the page a fresh `navigator`, so every emulation left only a resized window. The reload is gone, and so is the injected viewport meta, which desktop Safari ignores and which would have corrupted a later `safari_inspect_viewport` — together with the `scale` parameter it fed. An unknown device name is refused with the list instead of silently resizing to 375×812 while reporting the requested device, a custom user agent is escaped against backslash break-outs, and the result reports the viewport Safari actually produced — measured after showing the tab for a moment, because a background tab keeps its old layout until it is shown (on Safari 27 a resize to 402px left it at 1512px, media queries included), and noting when the screen is too short for the device's height. `safari_reset_emulation` puts back the window bounds from before the first emulation instead of forcing 1440×900, and removes only the getters emulation added, without reloading; it re-lays out the tab the same way.
- **Stylesheet scans see constructable stylesheets again on Safari 27.** Safari 27 stopped listing adopted style sheets in `document.styleSheets`, as the CSSOM spec requires (release notes, 174583340), so `safari_css_coverage`, `safari_webkit_compat` and the `env(safe-area-inset-*)` detection in `safari_safe_area_insets` silently lost every rule a web-component page ships that way. They now read `document.adoptedStyleSheets` as well, skipping any sheet an older Safari still lists, and coverage labels those sheets `(adopted)`.
- **`safari_check_pwa` reflects Home Screen web apps as iOS 26 and later ship them.** Every site added to the Home Screen now opens as a web app by default, with no installability requirements (WebKit, Safari 26.0), so a missing `apple-mobile-web-app-capable` no longer fails a page whose manifest sets the display mode, the manifest check no longer claims the manifest is "required for PWA install prompts" — Safari has none — and the theme-color check stops promising that Safari tints its tab bar with it: on iOS 27.0 a page with `theme-color: #ff0000` on a green background gets green bars, the page background and not the meta tag.
- **The Safari extension app builds with Xcode 27.** The project targeted macOS 10.14; Xcode 27 accepts deployment targets from 12.0 and stops the build with "The macOS deployment target 'MACOSX_DEPLOYMENT_TARGET' is set to 10.14", so the documented `xcodebuild` recipe failed. The target is now 12.0.
- **Setup instructions match Safari 27 and macOS 27.** "Allow JavaScript from Apple Events" and "Allow unsigned extensions" live in Safari → Settings → Developer; Safari 27 has no Develop-menu item for either, yet the README, `safari_doctor`, the install banner, CONTRIBUTING and SECURITY sent users there. macOS 27 renamed the Accessibility pane to "Device Control and Data Access" and the screen-capture pane reads "Screen & System Audio Recording"; both names now appear wherever the old ones did. `safari_doctor`'s Screen Recording line says the grant is needed by AppleScript screenshots and `safari_save_pdf` rather than by every screenshot (the extension captures without it), and under launchd it names the `node` binary to grant — a Homebrew node upgrade moves that path, and the grant does not follow. The README's comparison with Apple's `safaridriver --mcp` records that it ships in stable Safari 27.0 and that Apple's setup guide registers it under the name `safari-mcp`, and the troubleshooting table covers the macOS 27.0 issue where an unsigned extension does not appear until it is rebuilt (183044008).
...
- **`safari_screenshot_element` now works in a profile session.** The server routes it to the extension as `screenshot_element`, and both read-only command lists (the server's and the extension's) named that command, but the extension had no handler for it: every call came back `Unknown command: screenshot_element`. A profile session (`SAFARI_PROFILE`, as in the HTTP LaunchAgent) refuses the AppleScript fallback by design, so on that setup the tool could never return an image, while `safari_screenshot` on the same tab worked. The extension now selects the tab through the same helper as `screenshot` — so the two cannot drift apart — scrolls the element to the centre, captures the tab, gives the user's selected tab back, and crops the capture to the element: scaled by the capture's own width rather than `devicePixelRatio`, clipped to the viewport, and offset by the frame for a same-origin iframe match. A selector that matches nothing is now the error `Element not found: <selector>` instead of text wrapped in an image block, and the image is labelled by its own bytes — the AppleScript path's PNG crop had been sent as `image/jpeg`. `test/screenshot-element-extension.test.mjs` runs the new extension code against fake tabs, a fake page and a fake canvas, and fails whenever a command the server routes to the extension has no handler there. Found on macOS 27 / Safari 27; a code gap, not a Safari change. Manifest 2.10.13 — rebuild the extension app to pick it up.
- **`safari_click` reaches buttons inside closed shadow roots and same-origin iframes again, by ref, text or selector.** `safari_snapshot` listed both kinds of target, and the click answered `Element not found` for each (live audit, macOS 27 / Safari 27). Two faults stacked. The persistent click bridge runs in the top frame's ISOLATED world, which sees neither child documents nor closed shadow roots — a closed root is reachable only from MAIN, through the getter `content.js` installs at `document_start`, which is how clicks resolved these targets before the bridge replaced the MAIN-world lookup. The bridge's miss then fell to an all-frame fallback that walked open roots only, and that fallback never got its arguments: Safari's `scripting.executeScript` drops `undefined` entries from `args` and shifts the rest left, so a ref-only probe received the ref as its CSS selector and matched nothing (proved live: a text click for `#inner` clicked `#inner`). Every executor now sends `null` in place of `undefined`, and the fallback is one resolver, `_clickFrameAction`, that probes every frame including closed roots and clicks exactly once — in MAIN only when the match sits in a closed root, in ISOLATED otherwise, as before. Clicks the bridge can resolve never reach it. `test/frame-and-closed-shadow-targets.test.mjs` runs the real command cases against jsdom with an `executeScript` fake that drops `undefined` like Safari and hides page globals from ISOLATED. Manifest 2.10.14.
- **The same argument shift broke `safari_wait_for` by text.** The text arrived in the selector slot, so the wait timed out on text that was already on the page. `safari_double_click` and `safari_right_click` by coordinates route their arguments the same way; their coordinate checks now treat `null` as absent, so a lone `y` cannot turn into a click at `(0, y)`.
- **`safari_type_text` into a field inside a same-origin iframe no longer types into a different field.** The top frame's deep query found the iframe field, and every typing strategy then went through the top document's `execCommand` — into whichever top-frame field last had focus (reproduced: text meant for the frame's input landed in an input inside a closed shadow root on the parent page). A field that belongs to another document is now a miss in the top frame, and the existing frame fallback types inside its own document.
- **`safari_select_option` by ref reaches a `<select>` inside a closed shadow root.** `mcpFindRef` walked open roots only, fell back to the ref's stored coordinates and returned the shadow host; the script then threw on the host's missing `options`, and the tool printed an empty result with the value unchanged. `mcpCollectRoots` now uses the same closed-root getter, which also covers the other AppleScript-engine finders built on it.
- **With the content bridge gone, a click or fill no longer answers "Scheduled" for a target it cannot reach.** When a tab's content-script listener is invalid — every tab that was open during an extension update, and in the live audit after today's reinstall even fresh tabs — `sendContentCommand` scheduled a lookup of the top document only and answered `Scheduled mcp-content-click` before knowing whether it would find anything. Closed-root and iframe targets were skipped silently and neither fallback ran (reproduced: a fill into a closed-root field answered "Scheduled" and left the value unchanged). The target is now resolved before answering, and a miss comes back as `Element not found`, which sends a click through `_clickFrameAction` and a fill through the MAIN-world deep finder. A top-document target is still dispatched after the answer, as before. Manifest 2.10.15.
...
- **The profile-window opener launches Safari in the background first** when it is not running (never hidden — a hidden Safari has no reachable menu bar).
...
### Added
- **`SAFARI_MCP_RAISE_ON_NAVIGATE=1` lets navigation foreground Safari, for agents whose job is *showing* a page.** Voice assistants and demo drivers answer "open YouTube" by navigating, and the focus guard immediately handed the foreground back to the previous app — undoing the very raise the agent asked for. Under the flag, `navigate`/`navigateAndRead` raise the front window and `restoreFocusIfStolen` yields. Off by default: the background-operation posture, and every existing setup, is unchanged. Contributed by [@shinyobjectz](https://github.com/shinyobjectz) ([#96](https://github.com/achiya-automation/safari-mcp/pull/96)).
- **`safari_run_script` can safely recover OAuth popups opened from cross-origin iframes.** The new `clickAndOpenPopup` action accepts one selector or snapshot ref, proves that it resolves to exactly one visible element across all frames, installs a one-shot `window.open` capture in that frame's MAIN world, dispatches one click, and opens the captured HTTP(S) destination through the profile-scoped WebExtension with `active: false` and the exact owned source tab as `openerTabId`. Safari must confirm that direct opener relationship before the new tab receives ownership or a receipt; otherwise only that just-created tab is closed and the flow fails. CAPTCHA/challenge targets fail closed, ambiguous targets are never clicked, popup mutations are never retried, and the result exposes only origin+path plus the opaque tab receipt — never OAuth query or fragment data.
...
### Changed
- **Compatible dependencies brought current.** `ws` 8.21.1 → 8.21.3 (the only runtime dependency), and dev: `@types/node` 26.1.1 → 26.4.0, `eslint` 10.8.0 → 10.9.1, and `globals` 17.8.0 → 17.11.0. `jsdom` remains on 29.1.1 because version 30 drops Node 20, which this project still supports and tests.

### Fixed
- **The binary npm ships ran only on Apple Silicon Macs with macOS 26 — everyone else got "helper process exited" on every single tool call.** `safari-helper` is a prebuilt Mach-O, and it had been compiled with a bare `swiftc -O`, which stamps the *build* machine's architecture and SDK into the output: arm64-only, `minos 26.0`. dyld then refuses to load it on Intel Macs and on every macOS older than 26. Because the helper is spawned lazily on first use, there was no install-time error and no useful message — just a helper that died at launch behind every tool the user tried. Two people hit this independently from opposite directions ([#96](https://github.com/achiya-automation/safari-mcp/pull/96) reported the macOS 15 dyld rejection; [#98](https://github.com/achiya-automation/safari-mcp/pull/98) tried to add an x86_64 build). The shipped binary is now **universal (arm64 + x86_64) targeting macOS 13**, `scripts/build-helper.sh` pins both knobs so a future rebuild can't silently re-narrow them, and a new test in `test/helper-binary-portability.test.mjs` fails on a single-architecture or too-new binary — verified to fail against the binary this release replaces.
- **`readPage` reported an SPA that was still painting as an empty page.** `innerText` and the TreeWalker fallback both see an empty body mid-render, so the read returned `title + ""` and downstream agents concluded the page was broken. The read now gives the page up to ~2s (4 tries, 700ms apart) to produce any text before returning the honestly-empty result. Contributed by [@shinyobjectz](https://github.com/shinyobjectz) ([#96](https://github.com/achiya-automation/safari-mcp/pull/96)).
- **An OFF→ON toggle mid-`connect` left the old poll loop running beside its successor, and a dequeued command could execute twice over.** The extension had no way to tell an intentional reconnect's `AbortError` from a real one, so the invalidated attempt carried on with a still-true `isConnected`. Poll loops now carry a generation stamp that is bumped *before* the abort, an invalidated attempt hands off to one fresh generation after releasing the re-entrancy lock, and `_commandExecutionTail` holds sole execution ownership until the in-flight command's result POST completes — so the successor loop cannot dequeue a second non-idempotent command in parallel. A prepared reload handoff is also kept by object identity while its exact command is still pending, so an ACK lost after arming no longer blocks every future reload on a still-healthy worker.
- **A new tab's receipt could be minted against `about:blank`.** Safari can resolve `tabs.update()` with the requested URL while `tabs.get()` still reports the bootstrap URL, and a receipt issued in that window was unusable on the very next command. Receipt issuance now waits for Safari to actually expose the navigation.
- **A second retained Safari worker could steal the bridge from a healthy worker and immediately invalidate fresh tab receipts.** The first verified worker now keeps an activity-based lease; another worker is rejected without disabling the lease holder unless the holder is stale or `safari_reload_extension` grants that exact successor a short-lived, one-use handoff capability. Reload stores the capability only long enough to cross `runtime.reload()`, and the host arms it only after the old worker acknowledges the command. The profile handshake still carries a code-capability protocol marker, while reload remains tab-independent: it inherits no receipt, resolves no target tab, activates no window, and grants ownership to nothing.
- **A valid tab receipt could not recover an owned form after OAuth opened a second Safari window.** `switch_tab` reinterpreted the caller's index inside the popup/current window and rejected it even though the receipt had already resolved to one exact owned tab globally. Receipt-based switching now targets that concrete tab id directly across profile windows, accepts a receipt without an index, updates only the MCP session's logical target, and never activates or focuses a Safari tab/window. After an origin-changing OAuth redirect, argument-free `getReceipt` likewise prefers the exact cached/most-recent tab the session already owns across windows instead of the active tab in an older window. Index-only switching remains scoped to the session window, and neither recovery path mints a receipt for an unowned tab.
- **Opaque tab receipts were lost when Safari suspended the extension worker on SPAs or duplicate URLs.** Recovery required the tab's full URL digest to remain byte-identical and globally unique, so a `history.pushState` route change—or a second PartnerStack tab at the same URL—made the original receipt unusable after the worker woke. V3 receipts now bind to a per-Safari-run epoch plus the concrete tab/window identity, refresh URL identity after safe recovery, and keep the original origin boundary immutable. The epoch lives only in `storage.session`: worker suspension preserves it, a browser-session boundary invalidates it, and an unavailable or failed session store blocks authority instead of falling back to restart-persistent local state. Generation guards and serialized authority writes prevent races from republishing stale state, and every unowned write fails closed instead of mutating the active user tab.
- **The Node 20 CI job crashed before two jsdom-backed suites could load.** A bulk dependency update bypassed the existing Dependabot major-version guard and moved `jsdom` 29.1.1 → 30.0.1; jsdom 30 requires Node 22.22.2, 24.15.0, or 26+, while the project still promises and tests Node 20. The dev dependency is restored to 29.1.1, and a repository test now ties that ceiling to the Node 20 matrix row so a future manual bulk update fails with the compatibility reason instead of `webidl.util.markAsUncloneable is not a function` deep inside undici.
...
- **Native clicks silently did nothing on macOS 26 (issue #29).** `safari_native_click` posts a CGEvent, and on Tahoe those events are filtered on their way to a background window: the call returned success, the coordinate was verifiably correct, and the control simply never activated — the worst failure shape, since every caller believed it had clicked. `performNativeClick` now tries an Accessibility press first and only falls through to CGEvent when that finds nothing. The AX path does not travel the HID/CGEvent route at all; it asks the app to activate the control directly, and WebKit turns that into an `isTrusted` DOM click, which is what trust-gated forms actually require. Two helper commands come with it for the cases a coordinate cannot address: `axpress` presses a Safari button by title anywhere in the AX tree (immune to z-order and focus), and `axpressWindow` first locates a Safari window by title, raises it from inside the helper, then presses — so it targets the automation window by name rather than "window 1", and never routes through `osascript`. Both are helper-level commands; no MCP tool is wired to them yet.
- **`safari_doctor` kept steering callers away from a click path that now works.** The macOS 26 banner told every user to reach for `safari_evaluate` or extension clicks on trust-gated forms — advice written when raw CGEvent was the only path native clicks had. With the Accessibility press landing first, `safari_native_click` handles those forms itself, so the banner was sending people around a working door. It now says clicks are covered and scopes the remaining warning to native keyboard and hover, which do still travel CGEvent.

## [2.16.3] - 2026-08-23

### Fixed
- **The startup banner printed nothing at all from 2.16.2 onward.** The runtime tool count added in 2.16.2 read `server` at module scope, but `server` is a local of the per-session `buildServer()` factory — so the banner threw a `ReferenceError` that its own best-effort `catch` swallowed, silently taking the version line and the GitHub link with it. The banner is now a function that receives the server it counts, invoked from `buildServer()` and limited to once per process. CI additionally gates on ESLint's `no-undef`, which had already flagged the bad reference before it shipped; the rest of the lint backlog stays advisory.
- **`safari_switch_tab` could be talked into a tab the session never opened.** A tab that redirects away from the URL we registered (`/dashboard` → `/login`, any 302) stopped matching by URL, so the pre-check fell back to "is this index tracked?" — but `_openedTabs` is keyed by tab *index*, and Safari renumbers every index whenever any tab closes, so a stale key could name one of the user's tabs. The pre-check now requires the tracked index **and** the origin it was opened on, matching the second factor the extension already enforces. This is the only ownership check standing on the AppleScript fallback path.
