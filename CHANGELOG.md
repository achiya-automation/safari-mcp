...
### Fixed
- **A command sent right after the extension wakes is no longer lost.** Whenever Safari woke the extension worker, a second `connect()` followed a few seconds after the first had already succeeded: the minute alarm scheduled a retry while the startup connect was still in flight, and a successful connect never cancelled that retry. The duplicate invalidated the healthy poll loop, which then returned without running the command it had just been handed, and the host waited out 30s for an answer that could never come. The host's own keepalive check is sent 3s after each verification, so it landed in that window almost every time — the `keepalive tab check skipped: Extension timeout` line that filled the daemon log every minute — and so did the first command after an idle period, which is why `safari_reload_extension` kept timing out. Diagnostic logging caught it exactly: `list_tabs delivered 30s ago to worker a65e8e … last heartbeat never`, between two verifications of that worker four seconds apart. A successful connect now cancels any pending retry, the alarm does not schedule one while a connect is in flight, and a loop that a reconnect invalidates still runs a command it already received. `test/reconnect-drops-command.test.mjs` covers all three.
