...
### Fixed
- **A same-version Safari worker could survive an extension reinstall, steal the bridge back from the installed worker, and make `safari_reload_extension` impossible.** The profile handshake now carries a code-capability protocol marker and records a worker only after that marker passes, so an older cached worker cannot become verified or consume commands. Reload itself is now extension-maintenance rather than a tab command: it never inherits the active receipt, resolves no target tab, and checkpoints only authority the worker already holds before reloading. This breaks the stale-worker/forged-receipt loop without activating Safari or granting ownership to any tab.
...
- **Opaque tab receipts were lost when Safari suspended the extension worker on SPAs or duplicate URLs.** Recovery required the tab's full URL digest to remain byte-identical and globally unique, so a `history.pushState` route change—or a second PartnerStack tab at the same URL—made the original receipt unusable after the worker woke. V3 receipts now bind to a per-Safari-run epoch plus the concrete tab/window identity, refresh URL identity after safe recovery, and keep the original origin boundary immutable. Safari startup rotates the epoch and atomically clears persisted authority so tab-id reuse across browser runs cannot inherit a receipt. Generation guards and a separate authority-storage queue prevent startup races from republishing stale state, and every unowned write now fails closed instead of mutating the active user tab. Four executable race/reset tests join the SPA, duplicate-tab, origin, rotation, and persistence coverage; the full suite is 179/179.
